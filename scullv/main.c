...
int scullv_read_procmem(struct seq_file *m, void *v)
{
	...
	printk(KERN_INFO "scullv_read_procmem \n");
...
{
	printk(KERN_INFO "scullv_proc_open \n");
...
{
	printk(KERN_INFO "scullv_release \n");
	return 0;
}
...
{
	printk(KERN_INFO "scullv_follow \n");
	while (n--) {
		if (!dev->next) {
...
loff_t scullv_llseek (struct file *filp, loff_t off, int whence)
{
	...
	printk(KERN_INFO "scullv_llseek \n");
	switch(whence) {
	case 0: /* SEEK_SET */
		newpos = off;
		break;
...
static void scullv_setup_cdev(struct scullv_dev *dev, int index)
{
	...
	printk(KERN_INFO "scullv_setup_cdev \n");
...
void scullv_cleanup(void)
{
	...
	printk(KERN_INFO "scullv_cleanup \n");
