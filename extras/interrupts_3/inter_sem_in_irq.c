...
void tasklet_fn(unsigned long arg)
{
    ...
    if( (initialised  >= 60) && ( initialised % 10 == 0 ) ){ 
        ...
        printk(KERN_INFO "Acquring the semaphore in tasklet function");
        down(&my_sem);
        printk(KERN_INFO "Acqured the semaphore in tasklet function");
...
static irqreturn_t my_irq_handler(int irq, void *dev_id)
{
    ...
    */
   printk(KERN_INFO "Acquring the semaphore in interrupt handler");
   down(&my_sem);
   printk(KERN_INFO "Acqured the semaphore in interrupt handler");
    ...
    printk(KERN_INFO "Releasing the semaphore in interrupt handler");
    up(&my_sem);
    printk(KERN_INFO "Released the semaphore in interrupt handler");
