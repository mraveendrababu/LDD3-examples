...
void tasklet_fn(unsigned long arg)
{
    ...
    if( (initialised  >= 60) && ( initialised % 10 == 0 ) ){ 
        ...
        printk(KERN_INFO "Acquring the spinlock in tasklet function");
        spin_lock(&my_spin_lock);
        printk(KERN_INFO "Acquired the spinlock in tasklet function");
...
static irqreturn_t my_irq_handler(int irq, void *dev_id)
{
    ...
    */
    ...
    /*
        semaphore calls added 
    */
   printk(KERN_INFO "Acquring the spinlock in interrupt handler");
   spin_lock(&my_spin_lock);
   printk(KERN_INFO "Acqured the spinlock in interrupt handler");
    ...
    printk(KERN_INFO "Releasing the spinlock in interrupt handler");
    spin_unlock(&my_spin_lock);
    printk(KERN_INFO "Released the spinlock in interrupt handler");
